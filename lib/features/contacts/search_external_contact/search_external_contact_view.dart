...
import 'package:manager/features/contacts/search_external_contact/search_external_contact_vm.dart';
...
class SearchExternalContactView extends StatefulWidget {
  const SearchExternalContactView({super.key});

  @override
  State<SearchExternalContactView> createState() =>
      _SearchExternalContactViewState();
}

class _SearchExternalContactViewState extends State<SearchExternalContactView> {
  ...
  Widget build(BuildContext context) {
    return ViewModelBuilder<SearchExternalContactViewModel>.reactive(
      viewModelBuilder: () => SearchExternalContactViewModel(),
  ...
  Widget _buildSearchTextField(
    ...
    SearchExternalContactViewModel model,
  ) {
    return Container(
      decoration: BoxDecoration(
        color: AppColors.white,
  ...
  Widget _buildSearchContent(SearchExternalContactViewModel model) {
    if (model.isLoading) {
      return Shimmer.fromColors(
        ...
        child: ListView.separated(
          ...
          separatorBuilder: (BuildContext context, int index) {
            return SizedBox(height: AppSizes.h15);
          },
          itemCount: 10,
        ),
      );
      ...
      onTap: () async {
        ...
        showCustomActionDialog(
          ...
          onPrimaryButtonPressed: () async {
            try {
              final res = await _contactService.sendExternalContactRequest(
                id: employee.id ?? '',
              );
              if (res.isRight()) {
                Get.back();
          ...
          Container(
            ...
            child: Stack(
              ...
              children: [
                  ...
                  child: ClipOval(
                    child: Container(
                      ...
                      child: Container(
                        ...
                        child: Image.network(
                          employee.profilePhoto.prefixWithBaseUrl ?? '',
                          fit: BoxFit.cover,
                        ),
                      ),
                    ),
                ...
                if (employee.flag != null)
                  Positioned(
                    bottom: -4,
                    right: -4,
                    child: SvgPicture.network(
                      employee.flag?.prefixWithBaseUrl ?? '',
                      width: 17,
                      height: 17,
                    ),
                  ),
